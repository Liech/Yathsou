...
namespace Ahwassa {
    ...
    SSBOmat4::SSBOmat4(const std::string& name, const std::vector<glm::mat4>& content) : Uniform(name) {
    ...
    SSBOmat4::SSBOmat4(const std::string& name, size_t amount) : Uniform(name) {
      glGenBuffers(1, &_ssbo);
    ...
    void SSBOmat4::setData(const std::vector<glm::mat4>& content) {
      glBindBuffer(GL_SHADER_STORAGE_BUFFER, _ssbo);
    ...
    std::string SSBOmat4::toGLSL() const {
      std::string result = "";
