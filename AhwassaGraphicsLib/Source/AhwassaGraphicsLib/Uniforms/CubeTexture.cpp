...
namespace Ahwassa {
  CubeTexture::CubeTexture(const std::string& name, std::vector<std::unique_ptr<Iyathuum::MultiDimensionalArray<Iyathuum::Color, 2>>>& input) : Uniform(name) {
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGBA, (GLsizei)input[0]->getDimension(0), (GLsizei)input[0]->getDimension(1), 0, GL_RGBA, GL_UNSIGNED_BYTE, input[0]->data());
  ...
  bool CubeTexture::isTexture() const {
    return true; 
  }

  std::string CubeTexture::getType() const {
    return "samplerCube";
  }
