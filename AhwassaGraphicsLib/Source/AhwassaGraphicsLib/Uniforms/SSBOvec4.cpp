...
namespace Ahwassa {
    ...
    SSBOvec4::SSBOvec4(const std::string& name, const std::vector<glm::vec4>& content) : Uniform(name) {
    ...
    SSBOvec4::SSBOvec4(const std::string& name, size_t amount) : Uniform(name) {
      glGenBuffers(1, &_ssbo);
    ...
    void SSBOvec4::setData(const std::vector<glm::vec4>& content) {
      glBindBuffer(GL_SHADER_STORAGE_BUFFER, _ssbo);
      GLvoid* p = glMapBuffer(GL_SHADER_STORAGE_BUFFER, GL_WRITE_ONLY);
      memcpy(p, content.data(), sizeof(glm::vec4) * content.size());
    ...
    std::string SSBOvec4::toGLSL() const {
      std::string result = "";
