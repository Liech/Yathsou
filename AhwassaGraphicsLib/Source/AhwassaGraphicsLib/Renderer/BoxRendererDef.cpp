#include "AhwassaGraphicsLib/Renderer/BoxRendererDef.h"
...
namespace Ahwassa {
  struct BoxRendererDef::RenderVars {
  ...
  BoxRendererDef::BoxRendererDef(std::shared_ptr<Camera> camera) {
    _vars = std::make_shared<BoxRendererDef::RenderVars>();
  ...
  glm::vec3 BoxRendererDef::getLightDir()const {
    return _lightDir;
  }

  void BoxRendererDef::setLightDir(const glm::vec3& pos) {
    _lightDir = pos;
  }

  void BoxRendererDef::draw() {
  ...
  void BoxRendererDef::clear() {
    _instances.clear();
  }


  void BoxRendererDef::makeModelArray(size_t bufferSize) {
  ...
  void BoxRendererDef::makeShader() {
    ...
    std::string vertex_shader_source = R"(
      ...
      void main() {
        ...
        FragPos   = (models[gl_InstanceID] * vec4(position , 1.0)).xyz;
  ...
  void BoxRendererDef::shaderCall(const std::vector<glm::mat4>& mats, const std::vector<glm::vec3>&clr, size_t amount) {
  ...
  std::shared_ptr<Dot> BoxRendererDef::newDot(const glm::vec3& pos, float size, const Iyathuum::Color& clr) {
    std::shared_ptr<Dot> result = std::make_shared<Dot>(pos,size,clr);
    _instances.push_back(result);
    return result;
  }

  std::shared_ptr<Line> BoxRendererDef::newLine(const glm::vec3& start, const glm::vec3& end, float size, const Iyathuum::Color& clr) {
    std::shared_ptr<Line> result = std::make_shared<Line>(start, end, size, clr);
    _instances.push_back(result);
    return result;
  }

  std::shared_ptr<Box> BoxRendererDef::newBox(const glm::mat4& mat, const Iyathuum::Color& clr) {
    std::shared_ptr<Box> result = std::make_shared<Box>(mat,clr);
    _instances.push_back(result);
    return result;
  }

  void BoxRendererDef::makeGeometry() {
