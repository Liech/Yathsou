...
namespace Ahwassa {
  ...
  void Line::recalculate() {
    ...
    _matrix = glm::mat4(1);
    _matrix = glm::translate(_matrix, _start);

    glm::vec3 dir = glm::normalize(_end - _start);
    float angle = std::atan2(dir[0], dir[2]);
    _matrix = glm::rotate(_matrix, angle, glm::vec3(0, 1.0, 0));
    float angleZ = -std::asin(dir[1]);
    _matrix = glm::rotate(_matrix, angleZ, glm::vec3(1.0, 0.0, 0.0));

    _matrix = glm::scale(_matrix, glm::vec3(_size, _size, glm::length(_end - _start)));
    _matrix = glm::translate(_matrix, glm::vec3(-0.5, -0.5, 0));
  }
}