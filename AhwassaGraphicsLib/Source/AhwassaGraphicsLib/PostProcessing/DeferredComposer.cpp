...
namespace Ahwassa {
  DeferredComposer::DeferredComposer(Window* window, const glm::vec2& resolution) :r(window) {
    ...
    _resultCanvas = std::make_shared<Ahwassa::Rendertarget>("Result", resolution);
  ...
  void DeferredComposer::draw() {
    r.start();
    r.draw(*getResult(), Iyathuum::glmAABB<2>(glm::vec2(0,0), (glm::vec2)_resolution),true);
    r.end();
  }
}