...
namespace Suthanus {
  ...
  public:
    ...
    virtual std::shared_ptr<Box      > newBox      (const glm::vec3& pos, const glm::vec3& size, bool isDynamic)                                                = 0;
    virtual std::shared_ptr<Sphere   > newSphere   (const glm::vec3& pos, float     radius, bool isDynamic)                                           = 0;
    //virtual std::shared_ptr<Vehicle  > newVehicle(const glm::vec3& pos)                                                                            = 0;
    virtual std::shared_ptr<HeightMap> newHeightMap(const glm::vec3& pos, const Iyathuum::MultiDimensionalArray<unsigned short, 2>&, float height) = 0;
  };
}