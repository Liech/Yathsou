...
namespace Aezesel {
  namespace MagicaVoxImporter {
    void ChunkRGBA::read(Reader own, Reader child) {
      ...
      for (int i = 0; i < 256; i++) {
        std::vector<unsigned char> content = own.readChars(4);
        palette[i].r = content[0];
        palette[i].g = content[1];
        palette[i].b = content[2];
        palette[i].a = content[3];
      }
    }
    ...
    std::shared_ptr<ChunkRGBA> ChunkRGBA::getDefault() {
      ...
      for (int i = 0; i < 256; i++) {
        std::vector<unsigned char> bytes = int2Char(default_palette[i]);
        result->palette[i].r = bytes[0];
        result->palette[i].g = bytes[1];
