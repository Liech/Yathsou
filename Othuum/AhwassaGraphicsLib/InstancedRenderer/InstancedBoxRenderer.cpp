#include "InstancedRenderer/InstancedBoxRenderer.h"
...
namespace Ahwassa {
  struct InstancedBoxRenderer::RenderVars {
  ...
  InstancedBoxRenderer::InstancedBoxRenderer(std::shared_ptr<Camera> camera) {
  ...
  glm::vec3 InstancedBoxRenderer::getLightDir()const {
    return _lightDir;
  }

  void InstancedBoxRenderer::setLightDir(const glm::vec3& pos) {
    _lightDir = pos;
  }

  void InstancedBoxRenderer::draw() {
  ...
  void InstancedBoxRenderer::makeModelArray(size_t bufferSize) {
  ...
  void InstancedBoxRenderer::makeShader() {
  ...
  void InstancedBoxRenderer::shaderCall(const std::vector<glm::mat4>& mats, const std::vector<glm::vec3>&clr, size_t amount) {
  ...
  std::shared_ptr<InstancedDot> InstancedBoxRenderer::newDot(const glm::vec3& pos, float size, Iyathuum::Color clr) {
    std::shared_ptr<InstancedDot> result = std::make_shared<InstancedDot>(pos,size,clr);
    _instances.push_back(result);
    return result;
  }

  std::shared_ptr<InstancedLine> InstancedBoxRenderer::newLine(const glm::vec3& start, const glm::vec3& end, float size, Iyathuum::Color clr) {
    std::shared_ptr<InstancedLine> result = std::make_shared<InstancedLine>(start, end, size, clr);
    _instances.push_back(result);
    return result;
  }

  std::shared_ptr<InstancedBox> InstancedBoxRenderer::newBox(const glm::mat4& mat, Iyathuum::Color clr) {
    std::shared_ptr<InstancedBox> result = std::make_shared<InstancedBox>(mat,clr);
    _instances.push_back(result);
    return result;
  }

  void InstancedBoxRenderer::makeGeometry() {
